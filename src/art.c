 ...
 */
FILE * fp;
const char* logpath ="/home/czl/trie_index/sbench/LISK/log/art.log";
...
static void* recursive_insert(art_node *n, art_node **ref, const unsigned char *key, int key_len, void *value, int depth, int *old, int replace) {
    ...
    // fprintf(fp, "line 554: insert node:%lx, key:%s, depth:%d\n", n, key, depth);
    ...
    if (IS_LEAF(n)) {
        ...
        *ref = (art_node*)new_node;
        ...
        // fprintf(fp, "line 587: create new node:%lx, key1:%s, key2:%s\n", new_node, l->key, l2->key);
        return NULL;
    }
    ...
    if (n->partial_len) {
        ...
        *ref = (art_node*)new_node;
        ...
        char c = depth + prefix_diff < key_len ? key[depth+prefix_diff] : '\0';
        add_child4(new_node, ref, c, SET_LEAF(l));
        // fprintf(fp, "line 623: prefix mismatch create new node:%lx, key1:%s\n", new_node, l->key);
        return NULL;
    }
...
RECURSE_SEARCH:;
    ...
    art_leaf *l = make_leaf(key, key_len, value);
    char c = depth < key_len ? key[depth] : '\0';
    add_child(n, ref, c, SET_LEAF(l));
    return NULL;
}
